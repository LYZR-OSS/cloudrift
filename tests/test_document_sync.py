...
def recorder(monkeypatch):
    ...
    import cloudrift.document.cosmos_sync as cosmos_mod
    import cloudrift.document.documentdb_sync as docdb_mod

    monkeypatch.setattr(docdb_mod, "MongoClient", _RecordingClient)
    monkeypatch.setattr(cosmos_mod, "MongoClient", _RecordingClient)
    return _RecordingClient


def test_documentdb_uri_returns_pymongo_client():
    # Real PyMongo client (no recorder) — verifies the dispatch returns the right type.
...
def test_cosmos_account_key_builds_mongo_uri(recorder):
    get_mongodb_sync("cosmos", account="myacct", account_key="raw+key/with=special")
    inst = recorder.instances[-1]
    uri = inst.args[0]
...
def test_cosmos_connection_string_passed_through(recorder):
    ...
    get_mongodb_sync("cosmos", connection_string=cs)
    inst = recorder.instances[-1]
    assert inst.args == (cs,)
...
def test_documentdb_pool_kwargs_standardized(recorder, kwargs, expected_max, expected_min):
    get_mongodb_sync("documentdb", **kwargs)
    inst = recorder.instances[-1]
...
def test_cosmos_pool_kwargs_standardized(recorder, kwargs, expected_max, expected_min):
    get_mongodb_sync("cosmos", **kwargs)
    inst = recorder.instances[-1]
    assert inst.kwargs["maxPoolSize"] == expected_max
    assert inst.kwargs["minPoolSize"] == expected_min
...
def test_cosmos_connect_failure_wrapped(monkeypatch):
    ...
    with pytest.raises(DocumentConnectionError, match="Failed to connect to Cosmos DB"):
        get_mongodb_sync("cosmos", account="a", account_key="k")
